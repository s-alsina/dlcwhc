...
{*     {block name='product_category_name'}
    ...
    {block name='product_brand'}
        {if isset($product.manufacturer_name ) && $product.manufacturer_name != ''}
            <div class="product-brand text-muted productCartDlc"> <a href="{$product.url}" class="productCartDlcBrand">{$product.manufacturer_name}</a></div>{/if}
    {/block}

    {block name='product_name'}
        <h2 class="h3 product-title productCartDlcTitle">
    ...
    {block name='product_reference'}
        {if $product.reference != ''}
            <div class="product-reference text-muted productCartDlc"> <a href="{$product.url}" class="productCartDlcLabel">{$product.reference}</a></div>{/if}
    {/block}
    ...
    {block name='product_description_short'}
        <div class="product-description-short text-muted productCartDlc">
            <a href="{$product.url}" class="productCartDlcLabel">{$product.description_short|strip_tags:'UTF-8'|truncate:360:'...' nofilter}</a>
        </div>
    {/block}

    {block name='product_price_and_shipping'}
        {if $product.show_price}
            <div class="product-price-and-shipping productCartDlcCols">
